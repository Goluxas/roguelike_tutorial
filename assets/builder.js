Game.Builder = function(width, height, depth) {
	...
	for (var z=0; z < depth; z++) {
		...
		// Set up the regions array for each depth
...
Game.Builder.prototype._canFillRegion = function(x, y, z) {
	...
	if ( x < 0 || y < 0 || z < 0 ||
		 x >= this._width || y >= this._height || z >= this._depth) {
		return false;
	}
...
Game.Builder.prototype._fillRegion = function(region, x, y, z) {
	...
	while (tiles.length > 0) {
		...
		// Iterate through each neighbor, checking if we can use
		// it to fill and if so updating the region and adding it
		// to our processing list
...
// This removes all tiles at a given depth with a region number
// (turns them into wall tiles)
// Checks EVERY tile looking for region matches. Not the most
// efficient but keeps the code simpler
...
Game.Builder.prototype._setupRegions = function(z) {
	...
	// Itereate through all tiles searching for a tile that
...
// at a given depth and a region at the level beneath it
Game.Builder.prototype._findRegionOverlaps = function(z, r1, r2) {
	var matches = [];

	// Iterate through all tiles, checking if they respect the
	// region constraints and are floor tiles. We check that they
	// are floor to make sure we don't try to put two stairs on the
	// same tile.
	for (var x=0; x < this._width; x++) {
		for (var y=0; y < this._height; y++) {
...
Game.Builder.prototype._connectRegions = function(z, r1, r2) {
	...
	// Make sure there actually was an overlap
	if (overlap.length == 0) {
		return false;
	}
...
Game.Builder.prototype._connectAllRegions = function() {
	for (var z=0; z < this._depth - 1; z++) {
		...
		// then we try. We store connected properties as strings for
		// quick lookups.
		var connected = {};
		var key;
		...
		for (var x=0; x < this._width; x++) {
			for (var y=0; y < this._height; y++) {
				...
				if (this._tiles[z][x][y] == Game.Tile.floorTile &&
					this._tiles[z+1][x][y] == Game.Tile.floorTile &&
					!connected[key]) {
					// Since both tiles are floors and we haven't already
					// connected the two regions, try now.
